...
```{code-cell} ipython3
def prepare_imdb_dataset(num_words: int, oov_char: int = 2) -> tuple:
    ...
    # make sure that 0 encodes pad token
    index_from = 3
...
(x_train, y_train), (x_test, y_test) = prepare_imdb_dataset(num_words=vocab_size)
